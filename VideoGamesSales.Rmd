...
4. Quali siano i generi più venduti nelle varie "regioni"\
...
```
L'andamento delle vendite varia in modo (circa) proporzionale al variare del numero delle pubblicazioni di giochi.
...
figure
```
Diversa distribuzione per “regione”.

## Vendite per genere e "regione"
...
sales_dataset %>%
  ...
  scale_fill_brewer(palette = "Set3")
```
Mediamente, i generi più redditizi sono anche quelli più popolari.
...
ggplot(top10, aes(x = reorder(Publisher, Sum_Sales), fill = Publisher, y = Sum_Sales)) +
  ...
  theme(plot.title = element_text(face = "bold", size=16),
        ...
        legend.position = "none")
```
Gli editori con il maggior numero di pubblicazioni non sono sempre quelli con il maggior numero di vendite.
...
ggplot(platform, aes(x = reorder(Platform, Total_Games), fill = Platform, y = Total_Games)) +
  ...
  theme(plot.title = element_text(face = "bold", size=16),
        ...
        plot.subtitle = element_text(size=14))
```
Le piattaforme pù “mature” sono quelle con il maggior numero di giochi pubblicati.
...
group_by(Platform) %>% 
summarise(NASales = sum(NA_Sales), EUSales = sum(EU_Sales), JPSales = sum(JP_Sales), Other_Sales = sum(Other_Sales))
...
grid.arrange(p2, ncol = 1)
```
Le piattaforme con il maggior numero di vendite sono (circa) quelle con il maggior numero di giochi.
...
La classificazione identifica a quale pubblico sia destinato ogni singolo gioco (in base ai contenuti presenti in esso).\
